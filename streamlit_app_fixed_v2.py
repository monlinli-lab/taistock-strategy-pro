...
如果無法取得完整即時資料，請給出最合理的摘要與保守估計，並維持 JSON 結構完整。
""".strip()
...
def get_api_key() -> str:
    ...
    try:
        ...
        if "VITE_GEMINI_API_KEY" in st.secrets:
            return str(st.secrets["VITE_GEMINI_API_KEY"]).strip()
    except Exception:
        pass
    ...
    return str(st.session_state.get("manual_api_key", "")).strip()
...
def fetch_with_retry(url: str, payload: Dict[str, Any], api_key: str, retries: int = 4, delay: int = 1) -> Dict[str, Any]:
    headers = {
...
def safe_float(v: Any, default: float = 0.0) -> float:
    try:
        text = str(v).replace(",", "").replace("%", "").replace("元", "").strip()
        return float(text)
    except Exception:
        return default
...
def normalize_analysis_data(data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    merged["stockCharacter"] = {
        **DEFAULT_DATA["stockCharacter"],
        **(data.get("stockCharacter", {}) if isinstance(data, dict) else {}),
    }
    merged["dividend"] = {
        **DEFAULT_DATA["dividend"],
        **(data.get("dividend", {}) if isinstance(data, dict) else {}),
    }
    merged["scoreBreakdown"] = {
        **DEFAULT_DATA["scoreBreakdown"],
        **(data.get("scoreBreakdown", {}) if isinstance(data, dict) else {}),
    }
    merged["financials"] = {
        **DEFAULT_DATA["financials"],
        **(data.get("financials", {}) if isinstance(data, dict) else {}),
    }

    if not isinstance(merged.get("news"), list):
        merged["news"] = DEFAULT_DATA["news"]
    if not isinstance(merged.get("historicalPoints"), list) or not merged.get("historicalPoints"):
        merged["historicalPoints"] = DEFAULT_DATA["historicalPoints"]

    return merged


def query_gemini_stock_analysis(code: str, entry_price: str, shares_count: str, api_key: str, model_name: str) -> Dict[str, Any]:
    if not api_key:
        raise RuntimeError("尚未提供 Gemini API Key。請在側邊欄輸入，或於 Streamlit Secrets / 環境變數設定 GEMINI_API_KEY。")
    ...
    result = fetch_with_retry(get_endpoint(model_name), payload, api_key)
    raw_text = (
    ...
    if not raw_text:
        raise RuntimeError("Gemini 回傳空白內容，請確認 API Key、模型名稱或額度設定。")

    clean_json = raw_text.replace("```json", "").replace("```", "").strip()
    try:
        return normalize_analysis_data(json.loads(clean_json))
    except Exception:
        start = clean_json.find("{")
        end = clean_json.rfind("}")
        if start != -1 and end != -1 and end > start:
            return normalize_analysis_data(json.loads(clean_json[start:end + 1]))
        raise RuntimeError("Gemini 回傳格式不是有效 JSON。")
...
st.sidebar.markdown("### Gemini 設定")
st.sidebar.text_input(
    "Gemini API Key",
    type="password",
    key="manual_api_key",
    placeholder="未設 Secrets 時可直接貼在這裡",
    help="優先讀取 Streamlit Secrets 與環境變數；若都沒有，會使用這裡手動輸入的金鑰。",
)
model_name = st.sidebar.text_input("模型名稱", value=DEFAULT_MODEL_NAME, placeholder="gemini-2.5-flash")

api_key = get_api_key()
if api_key:
    st.sidebar.success("已偵測到 Gemini API Key")
else:
    st.sidebar.warning("尚未偵測到 API Key，目前仍可用展示資料開啟頁面。")
...
st.sidebar.markdown("---")
st.sidebar.caption("可用任一方式提供金鑰：Streamlit Secrets、環境變數 GEMINI_API_KEY、或側邊欄手動輸入。")
...
if run_scan:
    ...
    if not clean_code:
        ...
        st.warning("目前沒有 API Key，已改用展示資料開啟。請在左側填入 Gemini API Key 後再執行即時掃描。")
        st.session_state.analysis_data = DEFAULT_DATA
        st.session_state.last_code = clean_code or "2330"
    else:
        with st.spinner("正在同步三年趨勢與全網情資..."):
            try:
                data = query_gemini_stock_analysis(clean_code, entry_price, shares_count, api_key, model_name.strip() or DEFAULT_MODEL_NAME)
                ...
                st.info("已保留目前畫面資料。若持續失敗，請先確認 API Key、模型名稱、額度與網路連線。")

analysis_data: Dict[str, Any] = normalize_analysis_data(st.session_state.analysis_data)
...
with col1:
    st.markdown(f"<div class='metric-card'><div class='muted'>本益比</div><div style='font-size:30px;font-weight:900;color:white;'>{analysis_data.get('financials', {}).get('pe', '--')}</div></div>", unsafe_allow_html=True)
with col2:
    st.markdown(f"<div class='metric-card'><div class='muted'>營收年增</div><div style='font-size:30px;font-weight:900;color:white;'>{analysis_data.get('financials', {}).get('revenueGrowth', '--')}</div></div>", unsafe_allow_html=True)
with col3:
    div = analysis_data.get("dividend", {})
    st.markdown(f"<div class='metric-card'><div class='muted'>除權息預估</div><div style='font-size:22px;font-weight:900;color:#fecdd3;'>{div.get('exDate', '未公告')}</div><div style='font-size:16px;color:#fb7185;font-weight:800;'>{div.get('amount', '--')}</div></div>", unsafe_allow_html=True)
with col4:
    st.markdown(f"<div class='metric-card'><div class='muted'>股性標題</div><div style='font-size:22px;font-weight:900;color:white;'>{analysis_data.get('stockCharacter', {}).get('title', '--')}</div></div>", unsafe_allow_html=True)

if entry_price or shares_count:
    pcol1, pcol2, pcol3 = st.columns([1.3, 1.0, 1.8])
    with pcol1:
        st.markdown(f"<div class='section-card'><div class='muted'>當前部位戰績（{asset_stats['shares']} 張）</div><div style='font-size:42px;font-weight:900;color:{'#fb7185' if asset_stats['profit_percent'] >= 0 else '#34d399'};'>{asset_stats['profit_percent']:+.2f}%</div><div style='font-size:24px;font-weight:800;color:{'#fb7185' if asset_stats['total_profit_amount'] >= 0 else '#34d399'};'>{asset_stats['total_profit_amount']:+,.0f} TWD</div></div>", unsafe_allow_html=True)
    with pcol2:
        st.markdown(f"<div class='section-card'><div class='muted'>實時部位市值</div><div style='font-size:34px;font-weight:900;color:white;'>{asset_stats['total_market_value']:,.0f} TWD</div></div>", unsafe_allow_html=True)
    with pcol3:
        st.markdown(f"<div class='section-card'><div class='muted'>指揮官指令</div><div style='font-size:18px;font-weight:800;color:#e2e8f0;line-height:1.8;'>{analysis_data.get('portfolioStrategy', '--')}</div></div>", unsafe_allow_html=True)

c1, c2 = st.columns([2.2, 1.2])
with c1:
    st.markdown("### 📈 三年戰略走勢全景")
    chart_df = three_year_df.set_index("區間")[["價格"]]
    st.line_chart(chart_df, use_container_width=True)
    st.dataframe(three_year_df, use_container_width=True, hide_index=True)
...
with c2:
    ...
    st.bar_chart(bd_df, use_container_width=True)
    st.dataframe(bd_df.reset_index(), use_container_width=True, hide_index=True)

st.markdown("### 🔭 戰術行情預演（近期模擬）")
short_chart_df = short_term_df.set_index("月份")[["策略模擬價"]]
st.area_chart(short_chart_df, use_container_width=True)
st.dataframe(short_term_df, use_container_width=True, hide_index=True)

left, right = st.columns(2)
with left:
    st.markdown("### 🛡️ 兵策大師：股性診斷")
    st.markdown(f"<div class='section-card'><div style='font-size:30px;font-weight:900;color:white;margin-bottom:10px;'>「{analysis_data.get('stockCharacter', {}).get('title', '--')}」</div><div style='font-size:20px;line-height:1.8;color:#e2e8f0;font-weight:700;'>{analysis_data.get('stockCharacter', {}).get('comment', '--')}</div></div>", unsafe_allow_html=True)

    st.markdown("### 🗞️ 全域實時戰報")
    news_list = analysis_data.get("news", []) or []
    if news_list:
        for i, item in enumerate(news_list, 1):
            st.markdown(f"<div class='section-card'><div class='muted'>情報 {i}</div><div style='font-size:18px;font-weight:800;color:white;line-height:1.7;'>{item}</div></div>", unsafe_allow_html=True)
    else:
        st.info("目前沒有可顯示的新聞摘要。")

with right:
    st.markdown("### ⚠️ 情勢影響深度分析")
    st.markdown(f"<div class='risk-box'><div style='font-size:19px;line-height:1.9;color:#f8fafc;font-weight:700;'>{analysis_data.get('newsImpactAnalysis', '--')}</div></div>", unsafe_allow_html=True)

    st.markdown("### 📌 指揮官戰術指令")
    st.markdown(f"<div class='section-card'><div class='muted'>短期戰術指令</div><div style='font-size:18px;font-weight:800;color:#e2e8f0;line-height:1.8;'>{analysis_data.get('shortTermAnalysis', '--')}</div></div>", unsafe_allow_html=True)
    st.markdown(f"<div class='section-card'><div class='muted'>長期戰略展望</div><div style='font-size:18px;font-weight:800;color:#e2e8f0;line-height:1.8;'>{analysis_data.get('longTermAnalysis', '--')}</div></div>", unsafe_allow_html=True)
